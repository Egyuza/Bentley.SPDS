/*--------------------------------------------------------------------------------------+
|
|     $Source: /miscdev-root/miscdev/mdl/examples/imodelvisitor/MyElementGraphicsProcessor.cpp,v $
|
...
bool GetFileName(WStringR fileName)
    {
      ...
      MSWChar     wsXmlFilename[MAXFILELENGTH];
       // int         status;
         if ((SUCCESS == mdlDialog_fileCreate (xmlFilename, NULL, 0, "sample.xml", "*.xml", "c:\\ddrive\\data\\tmp\\", "Save As XML File"))
                 && (0 < strlen(xmlFilename)))
                 {
                 mdlCnv_convertMultibyteToUnicode(xmlFilename, -1, wsXmlFilename, MAXFILELENGTH);
                 
                 //wcscpy (m_graphicXMLFile,wsXmlFilename);
...
+---------------+---------------+---------------+---------------+---------------+------*/
    ...
    * @bsimethod                       @author BSI
    +---------------------------------------------------------------------------*/
    BentleyStatus MyElementGraphicsProcessor::_ProcessSurface (MSBsplineSurfaceCR surface) 
        { return SUCCESS;}
    /*--------------------------------------------------------------------------**//**
    ...
    * @bsimethod                       @author BSI
    ...
    //these will trigger the callbacks for the more sophisticated processors
     //these have been implemented in the header file since they are single line methods.
    /*--------------------------------------------------------------------------**//**
    * @description set the callback trigger for Producing GPArrays     
    * @param  isCurved is the GPA a curve 
    * @bsimethod                       @author BSI
    +---------------------------------------------------------------------------*/
   // bool MyElementGraphicsProcessor::_ProduceGPAs (bool isCurved) const  {return true;}

    /*--------------------------------------------------------------------------**//**
    * @description set the callback trigger for Producing Facets     
    * @param  isIndexedPoly is the facet array indexed polygons
    * @bsimethod                       @author BSI
    +---------------------------------------------------------------------------*/
   // bool MyElementGraphicsProcessor::_ProduceFacets (bool isIndexedPoly) const  {return true;}

    /*--------------------------------------------------------------------------**//**
    * @description set the callback trigger for Producing Solid bodies     
    * @param  isCurved is the body a curve 
    * @param  kernelType is the solids engine a Parasolid or ACIS
    * @bsimethod                       @author BSI
    +---------------------------------------------------------------------------*/
   // bool MyElementGraphicsProcessor::_ProduceBodies (bool isCurved, int kernelType) const  {
    //    return kernelType== KERNEL_TYPE_PSOLID;
    //    //return false;
    //    }
 //   };
